...
exports.signUp = catchAsync(async (req, res, next) => {
  ...
  const url = `${req.protocol}://${req.get("host")}/me/?j=${param}`;
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token || token === "null" || token === "undefined") {
    ...
    const queryString = Buffer.from(req.query.j ?? "","base64").toString("utf8");
    if (queryString?.split("=")[1] === "true") return next(); 
...
exports.isLoggedIn = async (req, res, next) => {
  try {
    ...
    req.headers.authorization.startsWith("Bearer") )|| req.cookies.jwt) {
      ...
      // we will use access token if request is routed through api
     const apiToken = req.headers.authorization?.split(" ")[1];

     
      // 2)  Verify the token
      const decoded = await promisify(apiToken ? apiToken: jwt.verify)(
        req.cookies.jwt,
...
exports.forgetPassword = catchAsync(async (req, res, next) => {
  ...
  // 3) Send password resst link to user email
  try {
